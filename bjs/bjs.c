...
static struct bjs_conf_t conf = { 0, 0, 0, 0, 0, -1, 0, 0};
...
/* Global vars for XCPU error handling */
static char *ename;
static int ecode;
...
void * malloc(size_t size) {
    ...
    if (!p) {
	syslog(LOG_ERR, "Out of memory allocating %ld bytes.", (long)size);
	abort();
    }
    return p;
}
...
const char *
bjs_job_req(struct bjs_job_t *j, const char *key, const char *dfl) {
    struct sexp_t *sx;
    sx = sexp_find_list(j->reqs, key, NULL);
...
int bjs_job_runnable(struct bjs_job_t *j) {
	...
	upcount = 0;
	for (i=0; i < j->pool->nodes->len; i++) {
	    nd = &j->pool->nodes->nodes[i];
	    if (i < bjs_nids && bjs_node_idx[i]->up) upcount++;
	}
	...
	if (sx) {
		...
		if (*check == 0) {
			...
			j2 = list_entry(j, struct bjs_job_t, list);
			if (j2->job_id == id)
			    return 0;
		    }
		}
...
void bjs_node_allocate(struct bjs_job_t *j, int node, int exclusive) {
	...
	syslog(LOG_ERR,"bjs_node_allocate: Invalide node number: %d",node);
	return;
    }
    ...
    if (!bjsnode->up) {
	syslog(LOG_ERR, "bjs_node_allocate: Allocating down node: %d", node);
	return;
    }
    ...
    if (exclusive) {
        xcpu_node_flush(bjsnode);
        xcpu_add_group(bjsnode, j->gid);
        xcpu_add_user(bjsnode, j->uid, j->gid);
    } else {
...
static struct Xpnodeset clean_set;

void bjs_do_clean(void) {
    int i, j, nprocs, killed_one;
    struct Xpproc *plist;

    for (i=0; i < clean_set.len; i++) {
        nprocs = xcpu_proc_list(&clean_set.nodes[i], &plist);

        for (j=0; j < nprocs; j++) {
            /* Hack to not try to kill root-owned */
            if (plist[j].xcpusid && plist[j].xcpujid)
                xcpu_proc_kill(&plist[j], SIGKILL);
        }
        
        /* TODO: Destroy process list
...
void bjs_clean_node(int node) {
    ...
    if (xp_nodeset_add(&clean_set, n->node)) {
...
void bjs_kill_node(int node) {
	...
	struct node_alloc_t *n = list_entry(l, struct node_alloc_t, jobs_list);
	next = l->next;
...
int bjs_node_usable(int node, long pri) {
	...
	struct node_alloc_t *na=list_entry(l, struct node_alloc_t, jobs_list);
	if (na->job->priority >= pri) return 0;
    }
    return 1;
}
...
void bjs_job_environment(struct bjs_job_t *j) {
	...
	len += sprintf(tmp+len, "%s,", bjs_node_idx[n->node]->node->name);
    }
    tmp[len-1] = 0;
    printf("NODES=\"%s\"\n", tmp); fflush(0);
    setenv("NODES", tmp, 1);
...
int set_job_ids(struct bjs_job_t *j) {
	...
	while (grp && ngroups < NGROUPS_MAX) {
	    for (i=0; grp->gr_mem[i] && ngroups < NGROUPS_MAX; i++) {
    ...
    if (setgroups(ngroups, (gid_t *)groups)) {
...
int set_job_io(struct bjs_job_t *j) {
	...
	if (!(f = popen(outfile+1, "w"))) {
	    syslog(LOG_INFO, "Failed to run user pipe command: %s\n", outfile);
	    return -1;
	}
	fd = fileno(f);
    } else {
	...
	if (fd == -1) {
	    syslog(LOG_INFO, "Failed to open user output file: %s\n", outfile);
	    return -1;
	}
...
int start_job_batch(struct bjs_job_t *j) {
	...
	if (conf.acctlog) fclose(conf.acctlog);
	setsid();
	if (set_job_ids(j))
	    exit(1);
	...
	if (chdir(j->dir)) {
	    fprintf(stderr, "chdir(\"%s\"): %s\n", j->dir, strerror(errno));
	    exit(1);
	}

	if (set_job_io(j))
	    exit(1);
	execl(j->shell, j->shell, "-c", j->cmdline, 0, (char *)NULL);
	exit(1);
    }
    /* Parent */
    j->pid = pid;
...
void bjs_job_free(struct bjs_job_t *j) {
    ...
    if (j->client)  j->client->job = 0;
    if (j->cmdline) free(j->cmdline);
    if (j->shell)   free(j->shell);
    if (j->dir)     free(j->dir);
    if (j->envp) {
	for (i=0; j->envp[i]; i++)
	    free(j->envp[i]);
...
int sexp_check_format(struct sexp_t *sx, const char *fmt) {
	...
	if (fmt[i] == 'V' && !sexp_is_value(sx)) return -1;
	if (fmt[i] == 'L' && !sexp_is_list(sx))  return -1;
    }
    if (fmt[i] || sx) return -1;
    return 0;
}
...
struct bjs_job_t *mkjob(struct client_t *c, struct sexp_t *jobsx) {
	...
	j->envp = malloc_chk(sizeof(char *) *
			     (sexp_length(sexp_nth(jobsx, JX_ENV))+1));
	for (i=0, sx = sexp_nth(jobsx, JX_ENV)->list; sx; sx = sx->next)
...
void wait_on_children(void) {
	...
	for (l = bjs_jobs.next; l != &bjs_jobs; l = l->next) {
	    ...
	    if (j->pid == pid) {
		if (verbose > 1)
		    syslog(LOG_DEBUG, "job %d exited.", j->job_id);
		bjs_job_remove(j);
		break;
	    }
	}
...
int persistent_job_load_single(int id) {
	 ...
	 * this case. */
	syslog(LOG_ERR,"Submission failure while restoring job %d.",j->job_id);
	bjs_job_free(j);
	return -1;
    }
    j->pool = p;
...
int persistent_job_load(void) {
    ...
    nents = scandir(conf.spooldir, &de, scandir_select,
		    (int (*)(const void *, const void *))scandir_compare);

    for (i=0; i < nents; i++) {
	id = strtol(de[i]->d_name, 0, 0);
	printf("Restoring job %d\n", id);
	persistent_job_load_single(id);
	if (id >= job_id) job_id = id+1;
    }
...
int persistent_job_remove(struct bjs_job_t *j) {
    ...
    sprintf(path, "%s/%d", conf.spooldir, j->job_id);
    if (unlink(path)) {
	if (errno == ENOENT) return 0; /* ok - already removed */
	syslog(LOG_ERR, "%s: %s", path, strerror(errno));
	return -1;
    }
    return 0;
...
int client_setup_socket(char *path) {
	...
	syslog(LOG_ERR, "Listen socket path too long.  Max length is %d",
	       (int) sizeof(addr.sun_path)-1);
	return -1;
    }

    if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
	syslog(LOG_ERR, "socket(AF_UNIX, SOCK_STREAM, 0): %s",strerror(errno));
	return -1;
    }
...
int client_accept(void) {
    ...
    size = sizeof(addr);
    fd = accept(conf.client_sockfd, (struct sockaddr *) &addr,
                (socklen_t *) &size);
    if (fd == -1) {
...
int submit_check_perm(struct client_t *c, struct bjs_pool_t *p) {
    int i;
    if (c->euid == 0) return 0;	/* root = ok :) */
    if (p->users) {
	for (i=0; p->users[i] != -1; i++)
	    if (p->users[i] == c->euid)
		break;
	if (p->users[i] == -1) return -1;
    }
...
int client_submit_job(struct client_t *c, struct sexp_t *jobsx) {
    ...
    if (verbose > 2) syslog(LOG_DEBUG, "Calling %s submit.\n",p->policy->name);
    if (p->policy->submit(p, j) != 0) {
	/* Policy module generates the error message to the client in
	 * this case. */
    ...
    if (bjs_job_is_interactive(j)) c->state = CL_WAIT_NODES;
...
struct sexp_t *pool_sexp(struct bjs_pool_t *p) {
	...
	if (p->nodes->len <= bjs_nids+1 && bjs_node_idx[i]) {
	    node = bjs_node_idx[i];
	    nodes_total++;
	    if (node->up) {
		nodes_up++;
		if (list_empty(&node->jobs)) nodes_free++;
	    }
	}
    }
	...
	sx2 = sexp_create_list(jid,uid,
			       j->cmdline ? j->cmdline : "(interactive)",
			       subtime, starttime, NULL);
...
int client_process_sexp(struct client_t *c, struct sexp_t *sx) {
	...
	if (verbose) syslog(LOG_DEBUG, "Received job submission.");
...
void bjs_client_error(struct client_t *cl, char *fmt, ...) {
    ...
    buf = sexp_string(sx);
    if (verbose > 2) syslog(LOG_DEBUG, "MSG to client: %s\n", buf);
...
int config_spooldir_callback(struct cmconf *cnf, char **args) {
    struct stat buf;
    if (tc.spooldir) free(tc.spooldir);
    ...
    if (buf.st_uid != 0) {
	syslog(LOG_ERR, "spooldir %s should be owned by root.", tc.spooldir);
	return -1;
    }
    if (buf.st_mode & 022) {
	syslog(LOG_ERR, "spooldir %s should should not be writable to anybody"
...
int config_socketpath_callback(struct cmconf *cnf, char **args) {
    if (tc.socketpath) free(tc.socketpath);
...
static
int config_statfsaddr_callback(struct cmconf *cnf, char **args) {
    char *s;
    if (tc.statfsaddr) free(tc.statfsaddr);
    
    s = strchr(args[1], '!');
    if (!s)
	syslog(LOG_ERR, "Invalid statfs server address %s. Use format "
	       "<address>!<port>.", tc.statfsaddr);

    tc.statfsaddr = strdup_chk(args[1]);
    return 0;
}
...
int config_pool_callback(struct cmconf *cnf, char **args) {
	...
	syslog(LOG_ERR, "%s:%d: Node pool names are limited to %d characters.",
	       cmconf_file(cnf), cmconf_lineno(cnf), MAX_POOL_NAME);
	return -1;
    }
    for (i=0; i < tc.npools; i++) {
	if (strcmp(tc.pools[i].name, args[1]) == 0) {
	    syslog(LOG_ERR, "%s:%d: A pool named %s already exists!",
...
int config_nodes_callback(struct cmconf *cnf, char **args) {
    ...
    for (i=1; args[i]; i++) {
	ns = xp_nodeset_list(tc.statfsaddr);
	if (!ns) {
	    sp_rerror(&ename, &ecode);
	    syslog(LOG_ERR, "Failed to get XCPU node set: %s", ename);
	    return -1;
	}

	if (xp_nodeset_filter_by_node(p->nodes, ns, args[i]) < 0) {
	    syslog(LOG_ERR, "%s:%d: Invalid node specification: %s",
		   cmconf_file(cnf), cmconf_lineno(cnf), args[i]);
	    return -1;
	}

        p->nnodes = p->nodes->len;
        if (!p->nnodes && verbose)
            syslog(LOG_DEBUG, "%s: No XCPU nodes added to the pool",
                   args[i]);
...
int config_policy_callback(struct cmconf *cnf, char **args) {
    ...
    if (p->policy->init &&
	p->policy->init(p) != 0) {	/* Initialize policy */
	syslog(LOG_ERR, "%s:%d: Policy initialization failed for pool %s"
	       "(policy %s)\n", cmconf_file(cnf), cmconf_lineno(cnf),
	       p->name, p->policy->name);
	return -1;
    }

    if (verbose) syslog(LOG_INFO, "Assigned policy %s to pool %s.",
...
int config_pool_option(struct cmconf *cnf, char **args) {
	...
	syslog(LOG_ERR, "%s:%d: pool policy (%s) does not take any options.\n",
...
struct cmconf_option config_opts[] =
    {{"spooldir",   1,  1, 0, config_spooldir_callback},
     ...
     {"statfsaddr", 1,  1, 0, config_statfsaddr_callback},
...
void config_free(struct bjs_conf_t *c) {
    ...
    struct bjs_pool_t *p;
    /* Strings */
    if (c->spooldir)   free(c->spooldir);
    if (c->policypath) free(c->policypath);
    if (c->socketpath) free(c->socketpath);
    if (c->statfsaddr) free(c->statfsaddr);
	...
	if (p->nodes)  xp_nodeset_destroy(p->nodes);
	if (p->users)  free(p->users);
	if (p->groups) free(p->groups);
    }
    if (c->pools) free(c->pools);

    /* Files */
    if (c->client_sockfd != -1) close(c->client_sockfd);
    if (c->acctlog)             fclose(c->acctlog);
}
...
int config_xfer(void) {
	...
	for (i=0; i < tc.npools; i++) {
		...
		if (tc.pools[i].policy->submit)
		    tc.pools[i].policy->submit(&tc.pools[i], j);
		break;
	    }
	}
	if (i == tc.npools) {
	    ...
	    syslog(LOG_ERR, "Job %d has no home after reconfiguration.",
		   j->job_id);
    ...
    for (i=0; i < conf.npools; i++) {
	p = &conf.pools[i];
	for (j=0; j < p->nodes->len; j++) {
	    node = bjs_get_node(j);
	    if (node) node->owned = 1;
	}
    }
	...
	if (bjs_nodes[i].owned == 1) {
	    ...
	    /* Release node... */
            xcpu_node_flush(bjs_nodes[i].node);           
	}
    }
...
int config_load(void) {
    ...
    tc.statfsaddr    = strdup_chk(DEFAULT_STATFS_ADDR);
    tc.client_sockfd = -1;
...
/*--------------------------------------------------------------------
 * XCPU interface routines
 *------------------------------------------------------------------*/

static
void xcpu_nodeset_to_bjs_nodes(struct bjs_node_t **nodes_, int *nnodes_,
    ...
    for (i=0; i < nnodes; i++) {
	xpnd = &ns->nodes[i];
	nodes[i].node = xp_node_create(xpnd->name, xpnd->addr, xpnd->arch,
				       xpnd->status, xpnd->numjobs);
	node_idx[i] = &nodes[i];
        nodes[i].up = 0;
...
static
int update_cluster_status(char *statfs_addr) {
    ...
    ns = xp_nodeset_list(statfs_addr);
    if (!ns) {
	sp_rerror(&ename, &ecode);
	syslog(LOG_ERR, "Failed to get XCPU node set: %s", ename);
	return -1;
    ...
    if (ns->len != bjs_nnodes) {
	change = 1;
    } else {
	for (i=0; i < ns->len - 1; i++) {
	    if (strcmp(ns->nodes[i].name, bjs_nodes[i].node->name)) {
		change = 1;
		break;
	    }
	}
    ...
    if (change) {
	xcpu_nodeset_to_bjs_nodes(&new_nodes, &nnodes, &new_node_idx, &nids, ns);
    ...
    for (i=0; i < bjs_nnodes; i++) {
	status = (strcmp(bjs_nodes[i].node->status, "up") == 0) ? 1 : 0;
	if (bjs_nodes[i].up != status) {
		...
		for (j=0; j < conf.npools; j++)
		    for (k=0; k < conf.pools[j].nodes->len; k++)
			if (!strcmp(conf.pools[j].nodes->nodes[k].name, 
				    bjs_nodes[i].node->name)) {
			    bjs_clean_node(k);
			    break;
			}
		change = 1;
...
void print_status(void) {
	...
	for (j=0; j < p->nnodes; j++) {
	    printf("    node %4d %4s\n", p->nodes[j],
		   ...
		   (bjs_nodes[p->nodes[j]].up ? "up" : "down") :
		   "INV");
	}
    }
    }
	...
	for (l2 = j->nodes.next; l2 != &j->nodes; l2 = l2->next) {
	    struct node_alloc_t *n = list_entry(l2, struct node_alloc_t, nodes_list);
...
int main(int argc, char *argv[]) {
    int c, down_nodes = 0;
    sigset_t sset;
    struct Xpnodeset *down_nodeset = xp_nodeset_create();
    ...
    if (config_file[0] != '/') {
	syslog(LOG_WARNING, "Configuration reload will likely fail because"
    ...
    /* Load configuration */
    if (update_cluster_status(tc.statfsaddr) == -1) {
	syslog(LOG_ERR, "Failed to read cluster status.");
	exit(1);
    }
	...
	if (nclients < DEFAULT_MAX_CLIENTS) {
	    ...
	    if (maxfd < conf.client_sockfd) maxfd = conf.client_sockfd;
	}
	...
	if (r > 0) {
	    ...
	    down_nodeset = xp_nodeset_list_by_state(conf.statfsaddr, "down(initializing)");
	    xp_nodeset_append(down_nodeset, 
			      xp_nodeset_list_by_state(conf.statfsaddr, "down(disconnected)"));
	    xp_nodeset_append(down_nodeset, 
			      xp_nodeset_list_by_state(conf.statfsaddr, "down(connect_failed)"));
	    xp_nodeset_append(down_nodeset, 
			      xp_nodeset_list_by_state(conf.statfsaddr, "down(read_failed)"));
	    xp_nodeset_append(down_nodeset, 
			      xp_nodeset_list_by_state(conf.statfsaddr, "down(no_contact)"));
		...
		if (chng) {
		    if (verbose) syslog(LOG_INFO, "Scheduler status change.");
		    for (i=0; i < conf.npools; i++) {
			p = &conf.pools[i];
	...
	if (do_config_reload) {
	    syslog(LOG_ERR, "Rereading configuration from %s", config_file);
	    if (config_load())
