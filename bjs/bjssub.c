...
int main(int argc, char *argv[]) {
	...
	case 'R':
		...
		*(p++) = 0;
	    ...
	    break;
	case 'r':
	    sexp_append_sx(reqs, sexp_create_list("restartable", NULL));
	    break;
	case 'D':		/* override working directory */
	    if (optarg[0] != '/') {
		fprintf(stderr, "The working directory (%s) must be an"
			" absolute path.\n", optarg);
		exit(1);
	    }
	    pwd = optarg;
	    break;
	case 'O':
	    sexp_append_sx(reqs, sexp_create_list("output", optarg, NULL));
	    break;
	case 'i':
	    interactive = 1;
	    break;
	...
	if (!pwd) {
		...
		if (!getcwd(pwd, PATH_MAX)) pwd = "/"; /* Fallback */
	    }
	}
	job = sexp_create_list("job", pool, shell, cmdline, pwd, maskstr,NULL);
    ...
    if (bjs_recv(&nodesx)) {
	fprintf(stderr, "Failed to read node list from the scheduler.\n");
	exit(1);
    }
	...
	execl(shell, shell, "-c", cmdline, 0, (char *)NULL);
