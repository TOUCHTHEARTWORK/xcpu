...
/* old code form Larry McVoy */
struct sockaddr_in somesin = {AF_INET};	/* the rest is null - unix will do it */
void
servesocket(int port){
	char    buf[2048];
	int     seq, newsock, sock;
	unsigned namelen;
	struct hostent *h;
	int	sockbuf = 128*1024;
	...
	somesin.sin_port = htons(port);
	if (bind(sock, (struct sockaddr *) &somesin, sizeof(somesin)) < 0) {
		perror("bind");
		exit(2);
	}
	namelen = sizeof(somesin);
	if (getsockname(sock,  (struct sockaddr *)&somesin, &namelen) < 0) {
		perror("getsockname");
		exit(3);
	}
	printf("Server bound to port %u \n",ntohs(somesin.sin_port));
	fflush(stdout);
	if (listen(sock, 1) < 0) {
		perror("listen");
		exit(4);
	...
	for ( ;;  ) {
		namelen = sizeof(somesin);
		if ((newsock = accept(sock, (struct sockaddr *) &somesin, &namelen)) < 0) {
			perror("accept");
			exit(5);
		}
		printf("server sees remote 0x%x \n", 
				somesin.sin_addr.s_addr);
		if (fork() == 0)
			break;
		close(newsock);
	}
	while (setsockopt(newsock, SOL_SOCKET, SO_SNDBUF, &sockbuf, 4)) {
		sockbuf -= 1024;
	}
	setsockopt(newsock, SOL_SOCKET, SO_RCVBUF, &sockbuf, 4);
	sockbuf = 1;
	/* I think there is something funny in here ... */
	dup2(newsock, 0);
	dup2(newsock, 1);
	close(newsock);
	serve(0,1);
...
main(int argc, char **argv)
{
	...
	/* so port is 0 which is the inetd case. If port is non-zero, well ... */
	if (port)
	  servesocket(port);
