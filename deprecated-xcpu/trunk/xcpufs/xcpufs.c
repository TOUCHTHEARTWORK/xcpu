...
session_decref(Xsession *xs)
{
	...
	xs->refcount--;
	ref = xs->refcount;
	wipe = !xs->refcount && xs->mode==Normal;
	pthread_mutex_unlock(&xs->lock);
...
//	npfile_incref(session);
	xs->file = session;
	create_file(session, "ctl", 0666, qpath | Qctl, &ctl_ops, NULL, &xs->ctl);
...
ctl_write(Npfilefid *fid, u64 offset, u32 count, u8 *data, Npreq *req)
{
	...
	xs = fid->file->parent->aux;
	ret = bufwrite(&xs->ctl, xs->ctl.size, count, data);
