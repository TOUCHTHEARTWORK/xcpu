...
void removeworker(Worker *worker) {
	...
	/* Workers can be deleted when close() is called.  This code ensures 
	   that the close routine  has not already cleaned up the worker.
	*/
	...
	debug(Dbgfn, "worker %p (%s:%d) is done\n", worker, worker->ip, worker->port);
...
servicetimeout(int sig) {
	if(datalen < datasize && servicetimeexts > 0) {
		...
		if(servicetimeexts <= 0) {
	        	/* failed downloading within allowed time */
			servicetimedone = 1;
			return;
		}
		else {
...
attemptfileread(char* address, Spuser *user, int port) {
	...
	/* maxretries = -1 will try forever */
 	while(maxretries == -1 || numretries < maxretries) {
		numretries++;
