...
static int xexec(Xsession *xs, char *exec, char *cwd);
...
xexec(Xsession *xs, char *exec, char *cwd)
{
	...
	char **argvtok, **envtok;
	int pip[2];
	char buf, *xcpupath, *execpath, b[128];
	Xsession *s;
	...
	snprintf(b, sizeof(b), "/mnt/xcpu/%d/fs", xs->sid);
	cwd = b;
	...
	pid = fork();
	if (pid == -1) {
		snprintf(b, sizeof(b), "cannot fork: %d", errno);
		sp_werror(b, errno);
		return 0;
	} else if (pid == 0) {
		/* child */
		if (*exec == '/')
			...
			execpath = sp_malloc(strlen(cwd) + strlen(exec) + 2);
			if (!execpath) {
				fprintf(stderr, "Not enough memory");
				exit(1);
			}

			sprintf(execpath, "%s/%s", cwd, exec);
		}
