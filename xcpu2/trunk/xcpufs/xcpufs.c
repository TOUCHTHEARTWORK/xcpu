...
usage()
{
	fprintf(stderr, "xcpufs: -h -d -s -m msize -M mntdir -p port -t tmpdir -a auth-file-name (e.g. /etc/xpuc/admin_key.pub)\n");
	exit(-1);
}
...
nsinit(void)
{
	...
	ret = mkdir_die("term", 0775);
	ret |= mkdir_die("sandbox", 0775);
	ret |= mkdir_die("sandbox/home", 0775);
	ret |= mkdir_die("sandbox/dev", 0775);
	ret |= mkdir_die("sandbox/proc", 0775);
	ret |= mkdir_die("sandbox/sys", 0775);
	if (ret)
		return ret;
	...
	snprintf(buf, sizeof(buf), "import $XCPUTSADDR %s/term\n", mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "bind %s/term/$XCPUARCHDIR %s/sandbox\n", mntpath, mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "bind %s/term/home %s/sandbox/home\n", mntpath, mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "bind /dev %s/sandbox/dev\n", mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "bind /proc %s/sandbox/proc\n", mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "bind /sys %s/sandbox/sys\n", mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	snprintf(buf, sizeof(buf), "chroot %s/sandbox\n", mntpath);
	bufwrite(&nsbuf, nsbuf.size, strlen(buf), buf);
	return ret;
}
...
main(int argc, char *argv[])
{
	...
	int nokeys;
	pid_t pid;
	char *s, *ename, mntopts[128], mntdir[256];
	char *afname;
	...
	while ((c = getopt(argc, argv, "nhdDrsup:t:m:M:x:a:")) != -1) {
		switch (c) {
		case 'D':
			spc_chatty = 1;
			break;
...
#warning RACE CONDITION AND FAILURE ON SETUP ON MOUNT
	if (!fork()) {
		...
		snprintf(mntdir, sizeof(mntdir), "%s/xcpu", mntpath);
		if (mount("127.0.0.1", mntdir, "9p", 0, mntopts) < 0) {
			perror("mount");
		}
		exit(0);
	}
